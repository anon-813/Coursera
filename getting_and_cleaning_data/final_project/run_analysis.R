...
# read in train files and combine into df_train.  remove intermediate steps.
...
rm(s_train, y_train, x_train)

# read in test files and combine into df_test.  remove intermediate steps.
...
library(dplyr)
df_grouped <- df %>% group_by(activity, subject)
tidydf <- summarise_all(df_grouped, funs(mean))
